...
class Fun extends CI_Controller {
    ...
    public function top() {
        $data = array('a' => $this->Attraction->by_category('fun'));
        $this->_loadheader($data['a'][0]['name']);
        $this->parser->parse('detail_fun', $data['a'][0]);
        $this->_loadfooter();
    }
    
    // Display all the attractions in this category to the allattractions view.
    public function all() {
        $this->_loadheader('Korea Tourism - Attractions');
        $data = array('attractions' => $this->Attraction->by_category('fun'));
    ...
    public function get($id) {
        ...
        $this->parser->parse('detail_fun', $data);
        $this->_loadfooter();
    }
}
